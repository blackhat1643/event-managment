...
class Mdl_subscribe extends CI_Model{
  ...
  public function get_all_with_pagi($id,$limit = 10,$start = 0,$w="") 
  {
    ...
    $query = "SELECT * FROM $this->_table WHERE is_deleted = '0' AND type = 'subscribe'  $w  ORDER BY id DESC $l";
    $d = ORM::for_table($this->_table)->raw_query($query)->find_array();

      if($d) { return $d; } else { return FALSE; }
  }

  public function get_search_count($w) {

      $query = "SELECT * FROM `$this->_table` WHERE is_deleted = '0' AND type = 'subscribe' $w ORDER BY id DESC";
