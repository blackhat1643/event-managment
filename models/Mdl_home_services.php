...
class Mdl_home_services extends CI_Model
{
	...
	public function __construct()
  {
	...
	private $_table = 'tbl_home_services';
...
public function get_all_with_pagi($id,$limit = 10,$start = 0,$w="") 
    ...
    public function get_users($limit = 10,$start = 0,$w) {
		if($w == ""){
			...
			$d = ORM::for_table('tbl_home_services')->raw_query($query)->find_many();
		}					
	...
	public function get_user($id) {
   	...
   	public function get_count($w = null) {
   			$query="SELECT * FROM tbl_home_services WHERE is_deleted = '0' $w ORDER BY id DESC";
           	$TotalData = ORM::for_table('tbl_home_services')->raw_query($query)->find_array();
			$d = count($TotalData);
