...
class Mdl_home_testimonial extends CI_Model
{
	...
	public function __construct()
  {
	...
	private $_table = 'tbl_home_testimonial';
...
public function get_all_with_pagi($id,$limit = 10,$start = 0,$w="") 
    ...
    public function get_users($limit = 10,$start = 0,$w) {
		if($w == ""){
			...
			$d = ORM::for_table('tbl_home_testimonial')->raw_query($query)->find_many();
		}					
	...
	public function get_user($id) {
   	...
   	public function get_count($w = null) {
   			$query="SELECT * FROM tbl_home_testimonial WHERE is_deleted = '0' $w ORDER BY id DESC";
           	$TotalData = ORM::for_table('tbl_home_testimonial')->raw_query($query)->find_array();
			$d = count($TotalData);
	...
	public function update_row($post)
  	{
	  	...
	  	if($d)
    	{
            ...
            $d->inserted_time           = date('Y-m-d H:i:s');
              ...
              $rand = rand();
              $this->sam->upload_image('image','themes/default/front/assets//images/testimonial/',$rand);
